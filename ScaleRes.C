...
void ScaleRes(unsigned imode)
{
  ...
  //get data
  const unsigned n_f=7;
  double ets[n_f]={10.,20.,40.,60.,100.,150.,200.};
  ...
  for(int i(0);i<n_f;i++) {
    ...
    sceName << "data/regressionResult_" << ets[i] << "GeV_d010.root";
    std::cout<<"opening "<<sceName.str().c_str()<<std::endl;
    TFile *f1 = new TFile(sceName.str().c_str());
    TH1F *AAA = static_cast<TH1F*>(f1->Get("h_dead")->Clone());
